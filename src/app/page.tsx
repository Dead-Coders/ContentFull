    ...
    <div className='bg-gray-700 grid md:grid-cols-4 sm:grid-cols-2 grid-cols-1 p-5 gap-5'>
      ...
      {blogs.items.map((blog: any) => (
        <div className=' bg-white p-5' key={blog.sys.id}>
          ...
          <h1 className=' text-3xl font-semibold p-2 text-black gap-2'>{blog.fields.title}</h1>
          <p className='text-md text-black'>{blog.fields.desc}</p>
          <p className='text-md text-2xl text-black'>Size: {blog.fields.size}</p>
          <h2 className=' font-bold text-black'>Rs: {blog.fields.price}</h2>
        </div>
      ))}
